...
class SVMClassifier(SVC):
    """
    A wrapper class for the Support Vector Classifier (SVC) from scikit-learn.

    This class extends the SVC class from scikit-learn, inheriting all its attributes and methods, and provides simplified methods for training and prediction.
    ...
    Args:
        ...
        **kwargs: Arbitrary keyword arguments passed to the SVC constructor.
    
    Methods:
        train(X, y): Train the SVC model using the provided data.
        predict(X): Predict using the trained SVC model.
    """

    def __init__(self, kernel="rbf", C=1.0, gamma='scale', **kwargs):
        """
        Initialize the SVMClassifier model.
